    ...
    <div className="w-full space-y-6 p-4" dir="rtl">
      <div>
        ...
        <div className="flex gap-2">
          <Button
            variant={filters.type === 'bid' ? 'primary' : 'outline'}
            className="flex-1"
            onClick={() => onFilterChange({ ...filters, type: 'bid' })}
          >
            مزاد
          </Button>
          <Button
            variant={filters.type === 'giveaway' ? 'primary' : 'outline'}
            className="flex-1"
            onClick={() => onFilterChange({ ...filters, type: 'giveaway' })}
          >
            هدايا
          </Button>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {sizes.map(size => (
            <Button
              key={size}
              variant={filters.sizes.includes(size) ? 'primary' : 'outline'}
              className="text-sm"
